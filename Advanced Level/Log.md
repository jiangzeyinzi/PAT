...
- A1046   为防止超时，应该在输入时就计算好sum和dis[i]（第1个点到第i个点的距离），去dis[i]和sum-dis[i]中间的最小值。


- A1047   直接使用<set>运行超时，而使用<vector>+sort()可以通过，既然name每一个字符可以确定下来‘A-Z’或‘0-9’，就可以想到字符串哈希。   本题可以直接构建字符串名字的二维数组，然后在每门课数组中存入字符串对应的下标，通过字符串下标来代替字符串本身进行排序。
- A1050   HashTable[128]
