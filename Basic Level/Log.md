...
- **B1055(集体照)**   字符串按字典序升序排列不能使用`a.name<b.name`，这句话的意思是按照字符串的地址大小进行比对，因为栈地址是从高地址到地址值，所以先声明的变量往往更大。要进行字典序排列得使用`strcmp(a.name, b.name)<0`
...
B1035(插入与归并)、B1045(快速排序)、B1055(集体照)