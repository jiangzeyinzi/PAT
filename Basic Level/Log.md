...
- B1019   运行超时可能是陷入了无限循环，考虑边界点的特殊值进行测试。本题的黑洞点6174就是一个特殊的点，将while改成do while即可解决。
    ...
    //把int数组转为字符串如1，2，3，4 -> "1 2 3 4"
    void toStr(int a[], char arr[])
    {
    	int tmp = sprintf(arr,"%d",a[0]);
    	for(int k=1; k<N; k++)
    		tmp += sprintf(arr+tmp," %d",a[k]);
    }
    ```
...
- **B1033(旧键盘打字)**   第2行输入的文字串非空可知第一行可能为空，只有“+”（代表上档键），使用hash计算各个字符的出现较为方便。

    使用strlen()最后一个测试点超时，`for(int i=0; i<strlen(s1); i++)`运行超时？？？

    使用其他均可通过1、`for(int i=0; s2[i]; i++)` 2、`for(int i=0; s2[i]!=0; i++)` 3、`for(int i=0; s2[i]!='\0'; i++)`

    以后遍历字符串尽量使用`for(int i=0; s2[i]; i++)`
...
- **B1035(插入与归并)**   把所有插入和归并排序的中间过程存入int二维数组中，写一个int数组的比较来判断是否相同。既然是中间过程，肯定至少需要一趟排序才能做比较。按照一般来写inserSort和mergeSort就行了，其中写mergeSort的非递归形式（递归与条件不符，递归不能保证2-2-2这样的形式，而是1-2-1-2）中间过程可以直接用sort来排序，可以省下写merge的时间。   实在找不出自己代码的错误就重写一遍吧。
...
- **B1045(快速排序)**   将排序与未排序的相应位置进行比对，主元的位置在数组对应位置值相等，并且比当前位前面所有的值都大。 （一开始只考虑了对应相等，没有考虑特殊情况下乱序的位置刚好是主元位置的巧合）  最后要加上`printf("\n");`不然当主元个数为0出现的情况下判定为“格式错误”。
...
- **B1055(集体照)**   字符串按字典序升序排列不能使用`a.name<b.name`，这句话的意思是按照字符串的地址大小进行比对，因为栈地址是从高地址到地址值，所以先声明的变量往往更大。要进行字典序排列得使用`strcmp(a.name, b.name)<0`，先输出最后一排调用函数`void order(Stu a[], int left, int m)`，再调用函数输出剩下排。   实在不知道哪里出问题了还是重新写一遍吧！！！
...
- **B1068(万绿丛中一点红)**   题目要求一个点与周围8个方向超过一定阈值，且这个点是唯一一个的情况。应该先使用map作为hash数组找出所有情况下唯一的点，再进行阈值的判断，如果先判断阈值在求唯一性会比较困难。8个方向可以先存储起来，如下。在做题过程中由于把if();后面多打了;和abs()的括号位置放错，导致调试过久。
